...
void main() {
	...
	A = Mat::zeros(A.rows*2, A.cols*2, CV_8UC3);  // fill zeros
	...
	//imshow("OriginalA", A);
	VideoWriter writer(outputpath + "Double Size Ratation Animation.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 15, Size(A.cols, A.rows));
	int max = B.cols;
	for (int i = 0; i <= max; i += 10)
	{
		Transform<TYPES::ROTATIONCENTRE>().transform(A, B, i);
		if (!A.empty()) writer << A; // save image
		NGBTLA003::backprint("Processing " + to_string(100 * i / max) + "%");
	}
	writer.release();
