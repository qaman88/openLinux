 ...
 * parallel.c
 *
 * This file provides the "shell" for Prac01.
...
int median_filter (int input_data [9][9]);
...
/*
 * median_filter Determines the median of 9x9 matrix input to it and returns that value
 * it works by sorting the each of the rows into order and then sorts the centre column
 * from that column it returns the centre value
*/
int median_filter (int input_data [9][9])
{
	int i, l, m;
    ...
    for (l = 0; l < 8 ; l++)
    {
        for (i = 0; i < 8; i++)
        {
            if (input_data [i] [5] > input_data [i+1] [5])
            {
                temp = input_data [i] [5];
                input_data [i] [5] = input_data [i+1] [5];
                input_data [i+1] [5] = temp;
            }
        }
    }
    return input_data [5][5];
}
